	...
	def __call__(self, raw_grads):
		...
		if self.predict_all_neuron:
			for i in range(bias.shape[0]):
				...
				interpreter_outputs, interpreter_mag_pred = self.part_a(interpreter_inputs)

				most_similar_idx = self.algo(self.feature_embeds, tf.squeeze(interpreter_outputs), tf.squeeze(interpreter_mag_pred))
