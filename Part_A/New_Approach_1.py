...
class Part_A_V2(tf.keras.models.Model):
	...
	def __init__(self, embedding_size, hidden_size, heads, query_size, name = "part_a"):
		...
		self.final_predictor_layer = tf.keras.layers.Dense(9, activation = "linear")
	...
	def call(self, inputs):
		...
		co_sim_matrix = tf.reshape(co_sim_matrix, (1, tf.shape(co_sim_matrix)[-1], tf.shape(co_sim_matrix)[-1]))

		embeds = tf.reshape(embeds, (1, -1, self.embedding_size))

		raw_weights = tf.reshape(raw_weights, (1, -1))

		raw_weight_gradients = tf.reshape(raw_weight_gradients, (1, -1))
		...
		expanded_weights = tf.reshape(weights, (1, -1, 1, 1))

		expanded_weight_gradients = tf.reshape(weight_gradients, (1, -1, 1, 1))
		...
		expanded_self_attention = tf.reshape(self_attention, (1, -1, 1, self.embedding_size))
		...
		axis_3_flattened_attention = tf.reshape(weighted_attention, (1, -1, tf.shape(weighted_attention)[-2] * self.embedding_size))

		for l in self.axis_3_hidden_layers:
			axis_3_flattened_attention = l(axis_3_flattened_attention)

		axis_2_flattened_attention = tf.reshape(axis_3_flattened_attention, (1, -1))
