from train_sequence_approach_10 import tf, np, tqdm
from helper_functions_10 import *
from base_learner_10 import *

class Train_Pipeline:

	def __init__(self, base_model, part_a, algo, neuron_number, feature_embeds, average_embeds, num_neurons, loss_function, optimizer):
		self.base_model = base_model
		self.algo = algo
		self.part_a = part_a
		self.neuron_number = neuron_number
		self.feature_embeds = feature_embeds
		self.average_embeds = average_embeds
		self.num_neurons = num_neurons
		self.loss_function = loss_function
		self.optimizer = optimizer
		self.interpreter_callback = Interpreter_Custom_Callback(self.base_model, self.part_a, self.algo, self.neuron_number, self.feature_embeds, self.average_embeds, self.num_neurons)
...
class Interpreter_Custom_Callback:
	...
	def __init__(self, base_model, part_a, algo, neuron_number, feature_embeds, average_embeds, num_neurons):
	...
	def __call__(self, raw_grads):
		...
		if self.predict_all_neuron:
			for i in range(bias.shape[0]):
				...
				most_similar_idx = self.algo(self.average_embeds, tf.squeeze(interpreter_outputs))
